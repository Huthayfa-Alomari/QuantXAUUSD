...
//|                                                                   |
//| REPAIR PASS rewrite of the scheduling and accounting core:        |
//|                                                                   |
//| 1. TRUE MULTI-TIMEFRAME SCHEDULER: independent new-bar flags for  |
//|    D1/H4/H1/M15/M5. Each strategy runs on its own Cadence() TF,   |
//|    not lumped into a single H1-only gate (this was silently       |
//|    starving ORB, which needs M5 bars). Position management and    |
//|    MAE/MFE tracking still run every tick.                         |
//| 2. Signals from faster cadences (e.g. M5 ORB) are cached with a   |
//|    freshness window and folded into aggregation whenever ANY      |
//|    cadence closes - not just H1.                                  |
//| 3. `OnTradeTransaction` now routes every exit deal through         |
//|    `CPositionManager::ProcessDeal()`, which is the single place   |
//|    that turns 1..N partial deals into exactly one `TradeResult`.  |
//| 4. Strategy attribution, entry ATR/ADX/spread, and composite      |
//|    score are captured into the `PositionContext` at open time -   |
//|    no more hardcoded fallback attribution at close time.          |
//+------------------------------------------------------------------+
...
CMarketStructure      g_StateStructure;    // dedicated instance for Market State telemetry (separate from any strategy's own internal structure engine)
...
// Per-timeframe new-bar tracking (repair spec: independent MTF scheduler).
...
// Cached last-known signal per strategy, with a freshness timestamp.
// A signal only contributes to aggregation while still fresh (repair
// spec: cadence-appropriate scheduling, not a single H1-only snapshot).
...
int OnInit()
  {
   ...
   // [RISK-INTEGRITY] One-time dump of the RAW broker symbol specification
   // used by the sizing math - printed once so a tick_size/tick_value/
   // digits mismatch is visible immediately, without waiting for a trade.
   ...
   if(InpResearchMode)
      g_Logger.Info("RESEARCH MODE ENABLED. NOT a full virtual execution simulator - it logs the setup that would have been taken (direction/entry/SL/TP/lot/score) but does NOT walk it forward tick-by-tick, so no partials/BE/trailing/MAE/MFE/final-R are simulated for research-mode setups. See README.");
...
void OnTick()
  {
   ...
   // Refresh market data every tick - cheap: each timeframe self-caches
   // internally and only does real work on ITS OWN new closed bar.
   if(!g_MarketData.RefreshAll())
      return;

   // Position management (partials/BE/trailing) and MAE/MFE tracking run
   // every tick, independent of signal-generation cadence.
   double atrH1ForManagement = g_MarketData.H1().IsReady() ? g_MarketData.H1().ATR(1) : 0.0;
   g_PositionManager.ManageAll(_Symbol, atrH1ForManagement);

   if(g_RiskEngine.ShouldForceCloseForWeekend())
      ForceCloseAllForWeekend();

   if(!g_MarketData.IsReady() || g_MarketData.H1().Bars() < QXE_MIN_BARS_REQUIRED)
      return;

   // ---- Independent per-timeframe new-bar detection ----
   ...
   if(!isNewD1 && !isNewH4 && !isNewH1 && !isNewM15 && !isNewM5)
      return; // nothing closed on any tracked timeframe this tick
   ...
   if(isNewH1)
     {
      ...
      if(PrintSignals)
        {
         g_Logger.Debug(StringFormat("Regime=%s Trend=%.1f Range=%.1f Vol=%.1f",
                         QXE_RegimeToString(state.regime), state.trendScore, state.rangeScore, state.volatilityScore));
         g_Logger.Info(StringFormat(
            "[REGIME] state=%s confidence=%.2f adxH4=%.1f atrPct=%.2f efficiency=%.2f d1H4Aligned=%s",
            EnumToString(state.meta.regime), state.meta.confidence, state.adxH4, state.atrPercentile,
            state.rangeEfficiency, state.d1H4Aligned ? "true" : "false"));
        }
     ...
     {
      // Regime is only recomputed on H1 close; reuse the last computed
      // regime for aggregation triggered by a faster cadence (e.g. M5 ORB)
      // within the same hour.
      g_RegimeEngine.Evaluate(GetPointer(g_MarketData), state); // cheap (H1 data unchanged) - keeps `state` self-consistent
      g_MetaClassifier.Classify(state);
     }
   ...
   for(int i = 0; i < STRAT_COUNT; i++)
     {
      ...
      bool cadenceClosed = (cad == PERIOD_D1 && isNewD1) || (cad == PERIOD_H4 && isNewH4) ||
                            (cad == PERIOD_H1 && isNewH1) || (cad == PERIOD_M15 && isNewM15) ||
      ...
      if(g_Strategies[i].Evaluate(GetPointer(g_MarketData), state, s))
        {
         string eligReason;
         EligibilityDecision decision = g_EligibilityEngine.Evaluate((StrategyType)i, state.meta, eligReason);
         g_lastEligibility[i] = decision;

         if(PrintSignals)
            g_Logger.Info(StringFormat("[ELIGIBILITY] strategy=%s decision=%s regime=%s reason=%s",
                           g_Strategies[i].Name(),
                           (decision == ELIGIBILITY_ALLOW) ? "ALLOW" : (decision == ELIGIBILITY_CONFIRMATION_ONLY) ? "CONFIRMATION_ONLY" : "BLOCK",
                           EnumToString(state.meta.regime), eligReason));

         if(decision == ELIGIBILITY_BLOCK)
           {
            g_lastSignal[i].direction = SIGNAL_NONE; // blocked strategies never enter aggregation
           }
         else
           {
            g_lastSignal[i] = s;
            ...
            anyFreshSignal = true;
            if(PrintSignals)
               g_Logger.Info(StringFormat("Signal: %s %s score=%.1f reason=%s",
                              g_Strategies[i].Name(), QXE_DirectionToString(s.direction), s.score, s.reason));
           }
        }
      else
        {
         // This strategy's cadence closed but it no longer qualifies -
         // clear any stale cached signal so it cannot be aggregated later.
         g_lastSignal[i].direction = SIGNAL_NONE;
        }
     }

   if(!anyFreshSignal)
      return; // no strategy produced anything new this bar-close

   // ---- Build the eligible signal set: only signals fresh within the
   //      current H1 bar's span are considered (bounds staleness to <= 1h) ----
   ...
   for(int i = 0; i < STRAT_COUNT; i++)
     {
      ...
      if(g_lastSignalBarTime[i] < g_lastBarH1)
         continue; // stale - generated before the current H1 bar opened
      eligibleSignals[eligibleCount] = g_lastSignal[i];
      confirmationOnly[eligibleCount] = (g_lastEligibility[i] == ELIGIBILITY_CONFIRMATION_ONLY);
      eligibleCount++;
     }
   ...
   TradeSetup setup = g_ScoreEngine.Aggregate(eligibleSignals, confirmationOnly, eligibleCount, state,
                                               GetPointer(g_InteractionMatrix));

   if(PrintSignals && setup.interactionReason != "")
      g_Logger.Info(StringFormat("[INTERACTION] contributors=%s modifier=%.1f reason=%s",
                     setup.contributingStrategies, setup.interactionModifier, setup.interactionReason));
   ...
   string spreadReject;
   double atrM5 = g_MarketData.M5().IsReady() ? g_MarketData.M5().ATR(1) : 0.0;
   if(!g_SpreadFilter.IsAcceptable(g_MarketData.CurrentSpreadPoints(), atrM5, _Symbol, spreadReject))
     {
   ...
   RiskParameters risk = g_RiskEngine.Validate(_Symbol, setup, equity);

   if(!risk.approved)
     {
      if(InpResearchMode)
         g_Journal.LogResearchSetup(setup, state, g_MarketData.CurrentSpreadPoints(), "rejected:" + risk.rejectReason);
      return;
     }

   if(InpResearchMode)
     {
      // NOTE: Research Mode currently logs the setup that WOULD have been
      // taken (deterministic, no lookahead) but does not yet walk it
      // forward as a full virtual position (partials/BE/trailing/MAE/MFE
      // simulated tick-by-tick). See README "Research Mode" for the exact
      // current scope - this is an honest limitation, not silently implied
      // to be a full simulator.
      g_Journal.LogResearchSetup(setup, state, g_MarketData.CurrentSpreadPoints(), "would_execute");
      g_Logger.Info(StringFormat("[RESEARCH] Setup logged: %s primary=%s score=%.1f entry=%.2f sl=%.2f lot=%.2f",
                     QXE_DirectionToString(setup.direction), QXE_StrategyToString(setup.primaryStrategy),
                     setup.compositeScore, setup.entry, setup.stopLoss, risk.lotSize));
      return;
     }
   ...
   if(ticket > 0)
     {
      g_PositionManager.RegisterOpen(ticket, setup, risk, state.atrH1, state.adxH1, state.spreadPoints, state);
      g_lastKnownRegime = state.regime;
     }
  }

//+------------------------------------------------------------------+
//| Trade transaction handler - routes every exit deal through the   |
//| position-context lifecycle in CPositionManager.                  |
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction &trans,
                         ...
                         const MqlTradeResult &result)
  {
   ...
   if(entryType != DEAL_ENTRY_OUT && entryType != DEAL_ENTRY_OUT_BY)
      return; // only exit deals close/reduce a position; ignore the opening deal here
   ...
   // DEAL_POSITION_ID is the stable logical-position identity that ties
   // the entry order and every subsequent partial/final deal together -
   // this is what CPositionManager's contexts are keyed on, NOT
   // CTrade::ResultOrder() (repair spec: "POSITION IDENTITY").
   ulong positionId = (ulong)HistoryDealGetInteger(trans.deal, DEAL_POSITION_ID);

   bool positionStillOpen = PositionSelectByTicket(positionId);

   TradeResult finalResult;
   if(g_PositionManager.ProcessDeal(positionId, dealVolume, dealProfit, dealPrice, dealTime, positionStillOpen, g_lastKnownRegime, finalResult))
     {
...
//+------------------------------------------------------------------+
//+------------------------------------------------------------------+
//| Config validation (Architecture v2 final integration, section 8) |
//+------------------------------------------------------------------+
bool ValidateDrawdownConfig(void)
  {
   ...
   if(!(InpModerateDDPct < InpHighDDPct))
     { ok = false; reason += "InpModerateDDPct must be < InpHighDDPct; "; }
   if(!(InpHighDDPct < InpCriticalDDPct))
     { ok = false; reason += "InpHighDDPct must be < InpCriticalDDPct; "; }
   if(!(InpCriticalDDPct < InpHardKillDDPct))
     { ok = false; reason += "InpCriticalDDPct must be < InpHardKillDDPct; "; }
   if(!(InpRecoveryExitDDPct < InpCriticalDDPct))
     { ok = false; reason += "InpRecoveryExitDDPct must be < InpCriticalDDPct; "; }
   if(!(InpCriticalCooldownDays >= 1))
     { ok = false; reason += "InpCriticalCooldownDays must be >= 1; "; }
   if(!(InpRecoveryRiskMultiplier >= 0.0 && InpRecoveryRiskMultiplier <= InpHighDDMultiplier &&
        InpHighDDMultiplier <= InpModerateDDMultiplier && InpModerateDDMultiplier <= 1.0))
     { ok = false; reason += "risk multipliers must satisfy 0 <= Recovery <= High <= Moderate <= 1; "; }

   if(!ok)
      g_Logger.Error("[RISK] CONFIG_ERROR INVALID_DD_THRESHOLDS " + reason);
...
void BuildMarketState(MarketState &state)
  {
   ...
   state.stdDev = (state.bbUpper - state.bbMiddle) / MathMax(InpBBDeviation, QXE_EPS);

   state.vwap = g_MarketData.VWAP();
   state.spreadPoints = g_MarketData.CurrentSpreadPoints();
   state.atrPercentile = 0.5; // filled by RegimeEngine.Evaluate()
   state.session = QXE_CurrentSession();

   state.regime = REGIME_NO_TRADE; // filled by RegimeEngine.Evaluate()
   state.trendScore = 0.0;
   state.rangeScore = 0.0;
   state.volatilityScore = 0.0;
   state.regimeScore = 0.0;

   // ---- Architecture v2 Market State Engine additions ----
   state.emaSlope20 = h1.EMA20SlopePoints(20);
   state.emaSlope50 = h1.EMA50SlopePoints(20);

   double closeD1 = d1.Close(1), ema200D1 = d1.EMA200(1);
   double closeH4 = h4.Close(1), ema200H4 = h4.EMA200(1);
   state.d1Bias = (closeD1 > ema200D1) ? 1 : ((closeD1 < ema200D1) ? -1 : 0);
   state.h4Bias = (closeH4 > ema200H4) ? 1 : ((closeH4 < ema200H4) ? -1 : 0);
   state.d1H4Aligned = (state.d1Bias != 0 && state.d1Bias == state.h4Bias);

   state.rangeEfficiency = QXE_AverageRangeEfficiency(h1, 10);
   state.trendEfficiency = QXE_TrendEfficiency(h1, 10);

   state.distEMA20_ATR = (state.atrH1 > QXE_EPS) ? (state.close - state.ema20) / state.atrH1 : 0.0;
   state.distEMA50_ATR = (state.atrH1 > QXE_EPS) ? (state.close - state.ema50) / state.atrH1 : 0.0;
   state.distEMA200_ATR = (state.atrH1 > QXE_EPS) ? (state.close - state.ema200) / state.atrH1 : 0.0;

   // Point-in-time correct: shift 1 = previous COMPLETED day; shift 0 =
   // today's forming bar, whose High/Low so far reflect only bars that
   // have actually happened - no lookahead.
   state.prevDayRange = d1.High(1) - d1.Low(1);
   double currDayRangeSoFar = d1.High(0) - d1.Low(0);
   state.currDayRangeRatio = (state.prevDayRange > QXE_EPS) ? currDayRangeSoFar / state.prevDayRange : 0.0;
   ...
   state.spreadATRRatio = (state.atrM5 > QXE_EPS) ?
      QXE_PointsToPrice(_Symbol, state.spreadPoints) / state.atrM5 : 0.0;
...
// Opposite-signal handling (spec section 44): never auto-reverses by
// default. When it does act, it explicitly tags the resulting close as
// EXIT_OPPOSITE_SIGNAL so statistics never guess (repair spec: "EXIT REASON").
void ForceCloseAllForWeekend(void)
  {
   ...
   if(dayAnchor == lastForceCloseDay)
      return; // already actioned today - avoid repeatedly trying every tick
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      SignalDirection posDir = (posType == POSITION_TYPE_BUY) ? SIGNAL_BUY : SIGNAL_SELL;

      if(newSetup.direction != SIGNAL_NONE && newSetup.direction != posDir && newSetup.compositeScore > 60.0)
        {
         g_Logger.Info(StringFormat("Opposite signal (score %.1f) - closing existing position %d",
                        newSetup.compositeScore, ticket));
         g_PositionManager.TagPendingExit(ticket, EXIT_OPPOSITE_SIGNAL);
         g_Executor.CloseFull(ticket);
        }
     }
  }
...
void LogHealthSnapshotOncePerDay(const MarketState &state)
  {
   ...
   double equity = AccountInfoDouble(ACCOUNT_EQUITY);
   g_RiskEngine.LogHealthSnapshot(state.regime, equity);
  }
//+------------------------------------------------------------------+
